                                       ...
                                       ble_gap_evt_adv_report_t const * const p_adv_report)
{
    ...
    // Imprimir la dirección del dispositivo anunciante
    ...
    NRF_LOG_DEBUG("Scan params: interval: %d, window: %d, timeout: %d, scan_phys: %d",
                  p_scan_ctx->scan_params.interval,
                  p_scan_ctx->scan_params.window,
                  p_scan_ctx->scan_params.timeout,
                  p_scan_ctx->scan_params.scan_phys);
    ...
    {
        NRF_LOG_DEBUG("addr_filter_enabled true");
        // Number of active filters.
        filter_cnt++;
        if (adv_addr_compare(p_adv_report, p_scan_ctx))
        {
            NRF_LOG_DEBUG("adv_addr_compare(p_adv_report, p_scan_ctx) true");
    ...
    {
        NRF_LOG_DEBUG("name_filter_enabled true");
        filter_cnt++;
        if (adv_name_compare(p_adv_report, p_scan_ctx))
        {
            NRF_LOG_DEBUG("adv_name_compare(p_adv_report, p_scan_ctx) true");
            filter_match_cnt++;
    ...
    {
        NRF_LOG_DEBUG("appearance_filter_enabled true");
        filter_cnt++;
        if (adv_appearance_compare(p_adv_report, p_scan_ctx))
        {
            NRF_LOG_DEBUG("adv_appearance_compare true");
            filter_match_cnt++;
            // Information about the filters matched.
...
{
    VERIFY_PARAM_NOT_NULL(p_scan_ctx);
    NRF_LOG_DEBUG("nrf_ble_scan_init start");
    p_scan_ctx->evt_handler = evt_handler;

#if (NRF_BLE_SCAN_FILTER_ENABLE == 1)
    NRF_LOG_DEBUG("HAS NRF_BLE_SCAN_FILTER_ENABLE 1");
    ...
    NRF_LOG_DEBUG("nrf_ble_scan_init finish");
    return NRF_SUCCESS;
}
...
ret_code_t nrf_ble_scan_start(nrf_ble_scan_t const * const p_scan_ctx)
{
    ...
    if (is_whitelist_used(p_scan_ctx))
    {
        NRF_LOG_DEBUG("is_whitelist_used(p_scan_ctx) true");
        if (p_scan_ctx->evt_handler != NULL)
        {
            NRF_LOG_DEBUG("p_scan_ctx->evt_handler != NULL true");
...
{
    NRF_LOG_DEBUG("nrf_ble_scan_params_set start");
    VERIFY_PARAM_NOT_NULL(p_scan_ctx);
    ...
    if (p_scan_param != NULL)
    {
        ...
        NRF_LOG_DEBUG("p_scan_param != NULL true");
        p_scan_ctx->scan_params = *p_scan_param;
    }
    else
    {
        NRF_LOG_DEBUG("p_scan_param != NULL false");
...
void nrf_ble_scan_on_ble_evt(ble_evt_t const * p_ble_evt, void * p_contex)
{
    ...
    // NRF_LOG_DEBUG("nrf_ble_scan_on_ble_evt start");
    // NRF_LOG_DEBUG(p_scan_data);
